...
from .biopython_utils import hotspot_residues, calculate_clash_score, calc_ss_percentage, calculate_percentages, calculate_clashes_after_superpostion_on_full_target
from .pyrosetta_utils import pr_relax, align_pdbs
from .generic_utils import update_failures, apply_advanced_rounding
from .ipsae_utils import calculate_ipsae

# hallucinate a binder
def binder_hallucination(af_model, design_name, starting_pdb, full_target_structure, full_target_pae, crop_structure_indices, chain, target_hotspot_residues, length, seed, helicity_value, design_models, advanced_settings, design_paths, failure_csv):
    ...
    # restart af model optimisation state if necessary
    if hasattr(af_model, "_opt"):
        print("with resetted model optimisation state and options")
        af_model.restart(keep_history=False, reset_opt=True)
    ...
    if advanced_settings["design_algorithm"] == '2stage':
        ...
        initial_plddt = get_best_plddt(af_model, length, advanced_settings)
        ...
        if initial_plddt > 0.65:
            ...
            if logits_iter > 0:
                ...
                logit_plddt = get_best_plddt(af_model, length, advanced_settings)
            ...
            if advanced_settings["temporary_iterations"] > 0:
                ...
                softmax_plddt = get_best_plddt(af_model, length, advanced_settings)
            else:
            ...
            if softmax_plddt > 0.65:
                ...
                if advanced_settings["hard_iterations"] > 0:
                    ...
                    onehot_plddt = get_best_plddt(af_model, length, advanced_settings)
    ...
    ### save trajectory PDB
    final_plddt = get_best_plddt(af_model, length, advanced_settings)
    af_model.save_pdb(model_pdb_path)
    ...
    if ca_clashes > 0:
        ...
        if final_plddt < 0.7:
            ...
            if binder_contacts_n < 3:
                ...
                # this is modified to check for clashes between binder and target crop and binder and full target.
                # When checking between binder and full target, we allow clashes if the are low plddt or pae with respect to the interface.
                if advanced_settings["save_superimpose_trajectory_structure"]:
                    superimposed_filepath = os.path.join(design_paths["Trajectory/Superimposed"], design_name+".pdb")
                else:
                    superimposed_filepath = None
                clash_dict = calculate_clashes_after_superpostion_on_full_target(model_pdb_path, 
                                                                                 full_target_structure, 
                                                                                 full_target_pae, 
                                                                                 crop_structure_indices, 
                                                                                 superimposed_filepath=superimposed_filepath,
                                                                                 clash_radius=1.5, 
                                                                                 clash_plddt_threshold=70.0,
                                                                                 clash_pae_threshold=15.0
                                                                                 )
                if clash_dict["clash_pass"] is False:
                    af_model.aux["log"]["terminate"] = "ClashingFullTarget"
                    update_failures(failure_csv, 'Trajectory_Clashes_FullTarget')
                    print("Clashes detected with full target structure, skipping analysis and MPNN optimisation")
                    print("")
                else:
...
def predict_binder_complex(prediction_model, binder_sequence, mpnn_design_name, target_pdb, chain, length, trajectory_pdb, prediction_models, advanced_settings, filters, design_paths, failure_csv, seed=None):
    ...
    for model_num in prediction_models:
        ...
        if not os.path.exists(complex_pdb):
            ...
            # numpy deep copy of the PAE matrix
            pae_matrix = np.copy(prediction_model.aux["pae"])

            # Calculate ipSAE from full PAE matrix
            target_len = prediction_model._target_len
            binder_len = prediction_model._binder_len
            ipsae_result = calculate_ipsae(pae_matrix, target_len, binder_len, advanced_settings=advanced_settings)
            ipsae_score = ipsae_result['ipSAE']
            print(f"Model {model_num+1} ipSAE score: {ipsae_score}")

            pae_matrices[model_num+1] = pae_matrix

            # extract the statistics for the model
            stats = {
                'pLDDT': apply_advanced_rounding(prediction_metrics['plddt'], advanced_settings), 
                'pTM': apply_advanced_rounding(prediction_metrics['ptm'], advanced_settings), 
                'i_pTM': apply_advanced_rounding(prediction_metrics['i_ptm'], advanced_settings), 
                'pAE': apply_advanced_rounding(prediction_metrics['pae'], advanced_settings), 
                'i_pAE': apply_advanced_rounding(prediction_metrics['i_pae'], advanced_settings),
                'ipSAE': apply_advanced_rounding(ipsae_score, advanced_settings),
            }
    ...
    return prediction_stats, pass_af2_filters, pae_matrices
...
def predict_binder_alone(prediction_model, binder_sequence, mpnn_design_name, length, trajectory_pdb, binder_chain, prediction_models, advanced_settings, design_paths, seed=None):
    ...
    return binder_stats, pae_matrices

# run MPNN to generate sequences for binders
def mpnn_gen_sequence(mpnn_model, trajectory_pdb, binder_chain, trajectory_interface_residues, advanced_settings):
...
def get_best_plddt(af_model, length, advanced_settings):
    return apply_advanced_rounding(np.mean(af_model._tmp["best"]["aux"]["plddt"][-length:]), advanced_settings)
...
def write_pae_json(pae_matrix: np.ndarray, out_json_path: str):
    """
    Write a full NxN PAE matrix to JSON in AlphaFold-compatible format.

    Parameters
    ----------
    pae_matrix : np.ndarray
        Square (N, N) PAE matrix (float32/float64).
    out_json_path : str
    ...
    payload = [{
        ...
        "max_predicted_aligned_error": float(pae.max()) if pae.size else 0.0
    }]
