...
from .generic_utils import clean_pdb, apply_advanced_rounding
from .biopython_utils import hotspot_residues

# Rosetta interface scores
def score_interface(pdb_file, binder_chain="B", advanced_settings=None):
    # load pose
...
def unaligned_rmsd(reference_pdb, align_pdb, reference_chain_id, align_chain_id, advanced_settings):
    reference_pose = pr.pose_from_pdb(reference_pdb)
    align_pose = pr.pose_from_pdb(align_pdb)
