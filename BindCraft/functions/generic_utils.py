...
def apply_advanced_rounding(value, advanced_settings, fallback_precision=2):
    """Round a numeric value based on advanced_settings[\"rounding_precision\"]."""
    precision = advanced_settings.get("rounding_precision", fallback_precision)

    if precision is False:
        return value

    if not isinstance(precision, int) or isinstance(precision, bool) or precision < 0:
        raise ValueError("'rounding_precision' must be a non-negative integer or false.")
...
def generate_directories(design_path):
    design_path_names = ["Accepted", "Accepted/Ranked", "Accepted/Animation", "Accepted/Plots", "Accepted/Pickle", "Accepted/Pae_json/Binder", "Accepted/Pae_json/Complex", "Accepted/Superimposed", "Trajectory",
                        "Trajectory/Relaxed", "Trajectory/Plots", "Trajectory/Clashing", "Trajectory/LowConfidence", "Trajectory/Animation",
                        "Trajectory/Pickle", "Trajectory/Superimposed", "Trajectory/ClashingFullTarget", "MPNN", "MPNN/Binder", "MPNN/Sequences", "MPNN/Relaxed", "Rejected"]
    design_paths = {}
...
def check_n_trajectories(design_paths, advanced_settings):
    # Go two levels up to get the "runs" directory
    base_dir = os.path.abspath(os.path.join(design_paths["Accepted"], "../.."))

    # Ensure we're in the expected directory
    if os.path.basename(base_dir) != "runs":
        raise ValueError(f"Expected base_dir to be 'runs', got '{base_dir}'")

    # Search for all .pdb files in runs/*/Trajectory/Relaxed/
    pattern = os.path.join(base_dir, "*", "Trajectory/Relaxed", "*.pdb")
    trajectories = glob.glob(pattern)
    if advanced_settings["max_trajectories"] is not False and len(trajectories) >= advanced_settings["max_trajectories"]:
        print(f"Target number of {str(len(trajectories))} trajectories reached, stopping execution...")
        return True
    else:
        return False
...
def check_accepted_designs(design_paths, mpnn_csv, final_labels, final_csv, advanced_settings, target_settings, design_labels):
    ...
    # Search for all .pdb files in runs/*/Accepted/
    pattern = os.path.join(base_dir, "*", "Accepted", "*.pdb")
    accepted_binders = glob.glob(pattern)

    if len(accepted_binders) >= target_settings["number_of_final_designs"]:
        print(f"Target number {str(len(accepted_binders))} of designs reached!")
        return True
    else:
        return False
...
def load_helicity(advanced_settings):
    if advanced_settings["random_helicity"] is True:
        ...
        helicity_value = apply_advanced_rounding(np.random.uniform(-3, 1), advanced_settings)
...
def perform_advanced_settings_check(advanced_settings, bindcraft_folder):
    ...
    # Configure global rounding precision.
    rounding_precision = advanced_settings.get("rounding_precision", 9)

    if isinstance(rounding_precision, str):
        lowered = rounding_precision.strip().lower()
        if lowered == "false":
            rounding_precision = False
        elif lowered.isdigit():
            rounding_precision = int(lowered)

    if isinstance(rounding_precision, float) and rounding_precision.is_integer():
        rounding_precision = int(rounding_precision)

    advanced_settings["rounding_precision"] = rounding_precision
...
def calculate_averages(statistics, advanced_settings, handle_aa=False):
    ...
    averages = {stat: apply_advanced_rounding(total / len(statistics), advanced_settings) for stat, total in sums.items()}

    # If we're handling aa counts, calculate their averages
    if handle_aa:
        aa_averages = {aa: apply_advanced_rounding(total / len(statistics), advanced_settings) for aa, total in aa_sums.items()}
