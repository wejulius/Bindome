...
OPTIONS=p:c:d:
LONGOPTIONS=pkg_manager:,cuda:,env_dir:
...
# Decide where the BindCraft environment lives
if [ -n "$env_dir" ]; then
  BINDCRAFT_ENV_PATH="${env_dir}/BindCraft"
else
  BINDCRAFT_ENV_PATH="${CONDA_BASE}/envs/BindCraft"
fi
echo -e "BindCraft environment path: $BINDCRAFT_ENV_PATH"

### BindCraft install begin, create base environment
echo -e "Installing BindCraft environment\n"
$pkg_manager create --prefix "$BINDCRAFT_ENV_PATH" python=3.10 -y || { echo -e "Error: Failed to create BindCraft conda environment"; exit 1; }
conda env list | awk '{print $NF}' | grep -Fx "$BINDCRAFT_ENV_PATH" >/dev/null 2>&1 || {
  echo -e "Error: Conda environment at '$BINDCRAFT_ENV_PATH' does not exist after creation."
  exit 1
}

# Load newly created BindCraft environment
echo -e "Loading BindCraft environment\n"
source ${CONDA_BASE}/bin/activate ${BINDCRAFT_ENV_PATH} || { echo -e "Error: Failed to activate the BindCraft environment."; exit 1; }
[ "$CONDA_PREFIX" = "$BINDCRAFT_ENV_PATH" ] || { echo -e "Error: The BindCraft environment is not active."; exit 1; }
echo -e "BindCraft environment activated at $BINDCRAFT_ENV_PATH"
...
if [ -n "$cuda" ]; then
  CONDA_OVERRIDE_CUDA="$cuda" $pkg_manager install \
    ...
    chex dm-haiku 'flax<0.10.0' dm-tree joblib ml-collections immutabledict optax requests python-igraph \
...
required_packages=(pip pandas libgfortran5 matplotlib numpy biopython scipy pdbfixer seaborn tqdm jupyter ffmpeg pyrosetta fsspec py3dmol chex dm-haiku dm-tree joblib ml-collections immutabledict optax jaxlib jax cuda-nvcc cudnn requests python-igraph)
missing_packages=()
...
echo -e "Activate environment using command: \"$pkg_manager activate $BINDCRAFT_ENV_PATH\""
echo -e "\n"
