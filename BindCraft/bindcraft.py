...
# print names of settings files being used
print("")
print("Using the following settings files:")
print(f"Basic settings: {settings_path}")
print(f"Filters: {filters_path}")
print(f"Advanced settings: {advanced_path}")
...
runtime_log = os.path.join(target_settings["design_path"], 'runtime_log.csv')
...
# fixed binder length
lengths = list(target_settings["lengths"])
if len(lengths) != 1:
    raise ValueError(f"One-length mode requires exactly one length, got: {lengths}")
length = int(lengths[0])


# 1) Design / hallucination model (protocol=binder)
af_design_model = mk_afdesign_model(
    protocol="binder",
    debug=False,
...
# 2) Complex validation model (protocol=binder)
...
# 3) Binder-alone validation model (protocol=hallucination)
...
# Lock monomer shapes once (fixed length)
...
# 5) full target structure and full target pae json for alignment and clash computation
parser = PDBParser(QUIET=True)
target_full_structure = parser.get_structure("target_full_structure", target_settings["full_target_structure"]) # 1-indexed
full_target_pae = np.array(json.load(open(target_settings["full_target_pae_json"]))[0]["predicted_aligned_error"])  # 0-indexed
crop_structure = parser.get_structure("crop_structure", target_settings["starting_pdb"]) # 1-indexed
target_crop_indices = set([
    res.get_id()[1]
    for res in crop_structure[0][target_settings["chains"]].get_residues()
    if res.get_id()[0] == " "
])
del crop_structure
...
while True:
    ...
    trajectory_dirs = ["Trajectory", "Trajectory/Relaxed", "Trajectory/LowConfidence", "Trajectory/Clashing", "Trajectory/Superimposed", "Trajectory/ClashingFullTarget"]
    ...
    if not trajectory_exists:
        # update runtime log with time stamp since last log entry, current design name, trajectory_number and accepted designs so far
        # always append to runtime log, except if empty then add header
        if not os.path.exists(runtime_log) or os.path.getsize(runtime_log) == 0:
            with open(runtime_log, 'w') as log_file:
                log_file.write("Timestamp(s),DesignName,TrajectoryNumber,AcceptedDesignsNumber\n")
        with open(runtime_log, 'a') as log_file:
            log_file.write(f"{int(time.time() - script_start_time)},{design_name},{trajectory_n},{accepted_designs}\n")

        print("Starting trajectory: "+design_name)

        ### Begin binder hallucination
        trajectory = binder_hallucination(af_design_model, design_name, target_settings["starting_pdb"], target_full_structure, full_target_pae, target_crop_indices, target_settings["chains"],
        ...
        trajectory_metrics = {
            k: apply_advanced_rounding(v, advanced_settings) if isinstance(v, float) else v
        ...
        if trajectory.aux["log"]["terminate"] == "":
            ...
            trajectory_interface_scores, trajectory_interface_AA, trajectory_interface_residues = score_interface(trajectory_relaxed, binder_chain, advanced_settings)
            ...
            trajectory_target_rmsd = target_pdb_rmsd(trajectory_pdb, target_settings["starting_pdb"], target_settings["chains"], advanced_settings)
            ...
            if advanced_settings["enable_mpnn"]:
                ...
                mpnn_trajectories = mpnn_gen_sequence(mpnn_model, trajectory_pdb, binder_chain, trajectory_interface_residues, advanced_settings)
                ...
                if mpnn_sequences:
                    ...
                    # restart prediction models if necessary
                    if hasattr(complex_prediction_model, '_opt'):
                        complex_prediction_model.restart(keep_history=False, reset_opt=True)
                    if hasattr(binder_prediction_model, '_opt'):
                        binder_prediction_model.restart(keep_history=False, reset_opt=True)
                    ...
                    for mpnn_sequence in mpnn_sequences:
                        ...
                        mpnn_score = apply_advanced_rounding(mpnn_sequence['score'], advanced_settings)
                        mpnn_seqid = apply_advanced_rounding(mpnn_sequence['seqid'], advanced_settings)
                        ...
                        mpnn_complex_statistics, pass_af2_filters, pae_matrices_complex = predict_binder_complex(complex_prediction_model,
                        ...
                        for model_num in prediction_models:
                            ...
                            if os.path.exists(mpnn_design_pdb):
                                ...
                                # analyze interface scores for relaxed af2 trajectory
                                mpnn_interface_scores, mpnn_interface_AA, mpnn_interface_residues = score_interface(mpnn_design_relaxed, binder_chain, advanced_settings)
                                ...
                                rmsd_site = unaligned_rmsd(trajectory_pdb, mpnn_design_pdb, binder_chain, binder_chain, advanced_settings)

                                # calculate RMSD of target compared to input PDB
                                target_rmsd = target_pdb_rmsd(mpnn_design_pdb, target_settings["starting_pdb"], target_settings["chains"], advanced_settings)
                        ...
                        mpnn_complex_averages = calculate_averages(mpnn_complex_statistics, advanced_settings, handle_aa=True)
                        
                        ### Predict binder alone in single sequence mode
                        binder_statistics, pae_matrices_binder_alone = predict_binder_alone(binder_prediction_model, mpnn_sequence['seq'], mpnn_design_name, length,
                        ...
                        for model_num in prediction_models:
                            ...
                            if os.path.exists(mpnn_binder_pdb):
                                rmsd_binder = unaligned_rmsd(trajectory_pdb, mpnn_binder_pdb, binder_chain, "A", advanced_settings)
                        ...
                        binder_averages = calculate_averages(binder_statistics, advanced_settings)
                        ...
                        statistics_labels = ['pLDDT', 'pTM', 'i_pTM', 'pAE', 'i_pAE', 'i_pLDDT', 'ipSAE', 'ss_pLDDT', 'Unrelaxed_Clashes', 'Relaxed_Clashes', 'Binder_Energy_Score', 'Surface_Hydrophobicity',
                        ...
                        if filter_conditions == True:
                            ...
                            # update number of accepted designs in runtime log, keep everything the same
                            if os.path.exists(runtime_log) and os.path.getsize(runtime_log) > 0:
                                runtime_df = pd.read_csv(runtime_log)
                                if "TrajectoryNumber" in runtime_df.columns and "AcceptedDesignsNumber" in runtime_df.columns:
                                    match = runtime_df["TrajectoryNumber"] == trajectory_n
                                    if match.any():
                                        runtime_df.loc[match, "AcceptedDesignsNumber"] = accepted_designs
                                        runtime_df.to_csv(runtime_log, index=False)

                            # copy designs to accepted folder
                            shutil.copy(best_model_pdb, design_paths["Accepted"])

                            # save best model pae json
                            write_pae_json(
                                pae_matrices_binder_alone[best_model_number],
                                os.path.join(design_paths["Accepted/Pae_json/Binder"], f"{mpnn_design_name}_model{best_model_number}.json")
                            )
                            write_pae_json(
                                pae_matrices_complex[best_model_number],
                                os.path.join(design_paths["Accepted/Pae_json/Complex"], f"{mpnn_design_name}_model{best_model_number}.json")
                            )

                            # superimpose best model binder onto full target structure and save
                            if superimpose_accepted_structure := advanced_settings.get("save_superimpose_accepted_structure", True):
                                superimpose_on_full_target_and_save_pdb(best_model_pdb, target_full_structure, target_crop_indices, target_settings, mpnn_design_name, best_model_number, design_paths)
